...
# note: variances in stata given for log scale and log shape; stata gives constant as exp(beta), therefore here using ln(cons) and ln(shape)
...
strategy.b<-c(current=0, optimal=1, atrisk=1, indemand=0, atrisk.thresh=0.9, indemand.thresh=0.7, lowhbdef.recall=12, vlowhbdef.recall=52, othdef.recall=4, prob.thresh=0.7, min.recall=12, curr.recall=12, length=52, thresh=13.5,  hb.min=8.6, hb.max=21.1) # hb.min/max from BLUPS file
